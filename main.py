...
while correct != len(states_with_pos):
    ...
    user_choice = screen.textinput(title=f"{correct}/{len(states_with_pos)} States/correct", prompt="Enter state:").strip().rstrip().title()

    # check if user already given that so that I don't increment
    ...
    # Check for exit 
    if user_choice == "Exit":
        missing_states = [state for state in states_with_pos.keys() if state not in correct_states]
...
# Exit code
# Then tell user which states they missed
screen.exitonclick()